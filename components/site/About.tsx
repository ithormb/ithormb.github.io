    ...
    <Band id="sobre" alt>
      <div className="grid gap-12 lg:grid-cols-[1.15fr_1fr] lg:items-start">
        <div>
          ...
          <Link href={`/${locale}/#experiencia`} className="mt-8 inline-flex h-11 items-center gap-2 rounded-lg border border-rule px-5 text-sm font-semibold text-ink transition-colors hover:border-ink-2 active:scale-95">
            {t("more_about", locale)} <span aria-hidden="true">→</span>
          </Link>
        </div>
        <dl className="grid grid-cols-2 gap-4 lg:mt-14">
          {site.stats.map((s) => (
            <div key={s.rotulo.pt} className="flex flex-col rounded-2xl border border-rule bg-surface p-5">
